...
<div x-data="{ showModalDelete: @entangle('showModalDelete') }" class="w-full overflow-hidden gap-8 p-8">
    ...
    <div class="mb-8">
        <a href="{{ $createRoute }}" class="bg-secondary py-2 px-4 border-[0.5px] border-third text-txtsecondary rounded-md">
            {{ $createButtonText }}
        </a>
    </div>
                    ...
                    <tr class="border-b-[1px] border-slate-600">
                        ...
                        <td class="sticky bg-primary text-start inset-0 h-full w-full">
                            <span class="w-full h-full flex justify-center items-center gap-2">
                                <button @click="showModalDelete=true; id={{ $item['id'] }}" wire:click="openModalDelete({{ $item['id'] }})" class="rounded-full bg-secondary/20 p-2">
                                    <img src="{{ asset('img/icon/trash-icon.svg') }}" class="w-4 h-4" alt="">
                                </button>
                                <a href="{{ route($routeEdit, ['id' => $item['id']]) }}" class="inline-block rounded-full bg-secondary/20 p-2">
                                    <img src="{{ asset('img/icon/edit-icon.svg') }}" class="w-4 h-4" alt="">
                                </a>
                            
                            <a href="{{ route('users-id', ['id' => $item['id']]) }}" class="inline-block rounded-full bg-secondary/20 p-2">
                                <img src="{{ asset('img/icon/edit-icon.svg') }}" class="w-4 h-4" alt="">
                            </a>
