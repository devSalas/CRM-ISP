<div x-data="{ showModalDelete: @entangle('showModalDelete'), selectedId: null, showModalTranslation:@entangle('showModalTranslation') }"class="mb-16">
                    ...
                    <tr class=" border-b-[1px] border-slate-300">
                        ...
                        <td class="sticky min-w-16 bg-white   text-start  inset-0 ">
                            ...
                            <div class="w-full h-full  flex gap-2">
                                <button class=" rounded-full bg-secondary/20 p-2 w-8 h-8 "
                                ...
                                <button class=" rounded-full bg-secondary/20 p-2 w-8 h-8 "
                                    @click="showModalTranslation=true;selectedId={{ $sale->id }}">
                                    <img src="{{ asset('img/icon/wrench-arrow.svg') }}" class="w-full h-full"
                                        alt="">
                                </button>
                            </div>
    ...
    <x-modal wire:model="showModalTranslation">
        <div class="p-8 flex flex-col gap-4">
            <p>¿Estás seguro de que deseas pasarlo a instalaciónes?</p>
            <div class="flex justify-end gap-4">
                <x-button @click="showModalTranslation = false">Cancelar</x-button>
                <x-danger-button wire:click="sendToInstallation(selectedId)">Estoy seguro</x-danger-button>
            </div>
            @if ($message)
                <div>{{ $message }}</div>
            @endif
